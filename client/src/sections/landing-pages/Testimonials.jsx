...
const Testimonials = ({ isHomepage = true }) => {
  ...
  // compute dynamic slide counts to avoid awkward empty space when we have < 3 items
  const total = testimonials.length;
  const showDesktop = Math.min(3, Math.max(total, 1));
  const showTablet = Math.min(2, Math.max(total, 1));
  // Ensure homepage carousel slides even when we only have up to 3 items
  const sliderTestimonials = isHomepage
    ? (total <= 3 ? [...testimonials, ...testimonials] : testimonials)
    : testimonials;
    ...
    <Box sx={{ py: 8, bgcolor: '#f7f9f8' }}>
      <Container maxWidth="xl" disableGutters={isHomepage}>
        ...
        {isLoading ? (
          ...
          // remove extra side margins so the slider uses the full container width
                        ...
                        <Box sx={{ position: 'relative', width: '100%', flexGrow: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                          ...
                          <Typography
                            ...
                            sx={{ 
                              ...
                              lineHeight: 1.8,
                              mb: 1,
                              // Ensure consistent box heights even with short text
                              minHeight: 96
                            }}
                          >

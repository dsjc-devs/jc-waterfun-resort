      ...
      {viewMode === 'grid' && (
        <React.Fragment>
          ...
          <MainCard>
            <Grid container spacing={2}>
              {paginatedUsers.map((_user) => {
                return (
                  <Grid item key={_user.userId} xs={12} md={4}>
                    <SimpleUserCard
                      user={_user}
                      currentUser={user}
                      onView={(user) => navigate(`/portal/staffs?userId=${user.userId}`)}
                      onEdit={(user) => navigate(`/portal/staffs?userId=${user.userId}&isEditMode=true`)}
                      onDelete={(user) => setDeleteConfigs({ open: true, userId: user.userId })}
                    />
                  </Grid>
                )
              })}
          ...
          <Box display="flex" justifyContent="center" mt={2}>
            <Pagination
              count={Math.ceil((users?.length || 0) / itemsPerPage)}
              page={page}
